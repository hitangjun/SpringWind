...
CREATE TABLE `cron_task` (
  ...
  `task_clazz_name` varchar(100) NOT NULL COMMENT '任务执行类名',
  `task_cron_time` varchar(100) DEFAULT NULL COMMENT '定时任务时间表达式',
  `task_params` varchar(100) DEFAULT NULL COMMENT '以空格分隔的任务参数',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_time` datetime DEFAULT NULL COMMENT '修改时间',
  `remark` varchar(120) DEFAULT NULL COMMENT '描述',
  `status` int(1) DEFAULT NULL COMMENT '状态:0无效，1可用',
   PRIMARY KEY (`id`),
  UNIQUE KEY `unique_index_for_cron_task` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='定时任务配置信息';
