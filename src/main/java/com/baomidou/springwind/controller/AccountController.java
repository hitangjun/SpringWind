...
//			try {
//				quartzJobManager.addJob(Pinyin4jHelper.converterToAllFirstSpell("URL抓取2"), CrawlJob.class,"* */5 * * * ?");
//				quartzJobManager.addJob(Pinyin4jHelper.converterToAllFirstSpell("URL抓取"), CrawlJob.class,"*/28 * * * * ?");
//			} catch (Exception e) {
...
//			}
			...
			if(USE_CAPTCHA){
				 isCaptchaOK = StringUtils.isNotBlank(ctoken)
	...
	public String lockscreen(Model model, String password) {
		...
		if (StringUtils.isBlank(loginName)) {
			...
			SSOHelper.clearLogin(request, response);
		} else if (isNotBlank(password) && isPost()) {
			/*
			 * 锁定页面登录
			 */
			User user = userService.selectByLoginName(loginName);
